      ...
      <?php foreach ($instances as $id => $inst): ?>
        ...
        <div class="instance-card <?= $statusClass ?> <?= $autoReplyClass ?> <?= $serverRunning ? 'is-running' : '' ?> <?= $whatsappConnected ? 'whatsapp-connected' : '' ?> <?= $isSelected ? 'is-selected bg-light' : 'bg-white hover:bg-light' ?> block w-full p-3 rounded-xl border transition">
          ...
          <div class="mt-3 flex flex-wrap justify-end gap-2">
            ...
            <a
              href="grupos.php?instance=<?= urlencode($id) ?>"
              class="text-[11px] px-2.5 py-1 rounded-full border border-slate-300 text-slate-600 flex items-center gap-1 hover:bg-slate-100 transition"
            >
              <svg class="w-3.5 h-3.5" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
        ...
        <div>
          ...
          <?php if ($selectedInstanceId): ?>
            <div class="mt-4 flex flex-wrap gap-2 text-[11px]">
              <a
                href="conversas.php?instance=<?= urlencode($selectedInstanceId) ?>"
                class="px-2.5 py-1 rounded-full border border-primary/60 text-primary flex items-center gap-1 hover:bg-primary/10 transition"
              >
                <svg class="w-3.5 h-3.5" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                  <path d="M3 4.5A1.5 1.5 0 014.5 3h11A1.5 1.5 0 0117 4.5v6A1.5 1.5 0 0115.5 12H8l-4 4V4.5z"></path>
                </svg>
              ...
              <a
                href="grupos.php?instance=<?= urlencode($selectedInstanceId) ?>"
                class="px-2.5 py-1 rounded-full border border-slate-300 text-slate-600 flex items-center gap-1 hover:bg-slate-100 transition"
              >
                <svg class="w-3.5 h-3.5" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                  <path d="M7 7a3 3 0 116 0v1h1a2 2 0 012 2v5a2 2 0 01-2 2H6a2 2 0 01-2-2v-5a2 2 0 012-2h1V7z"></path>
                </svg>
...
5. Separe o texto destinado ao usuário das instruções/funções com `&&&`; o que vier depois do marcador será tratado como comandos e não será enviado ao WhatsApp.
      ...
      <section id="calendarSettingsSection" class="bg-white border border-mid rounded-2xl p-6 mt-6">
      ...
      <section class="mt-4 rounded-2xl border border-mid bg-gradient-to-b from-slate-900 via-slate-900 to-slate-800 p-6 text-white shadow-xl">
        <div class="font-semibold text-lg mb-2">Controle visual</div>
        <p class="text-sm text-white/70">
          Organize seus calendários, defina disponibilidade e acompanhe o status da integração antes de seguir para o histórico de conversas.
        </p>
        <ul class="mt-4 space-y-3 text-sm">
          ...
          <li class="flex items-center gap-3">
            ...
            Sincroniza automaticamente os calendários exibidos
          </li>
          <li class="flex items-center gap-3">
            <span class="inline-block h-2 w-2 rounded-full bg-sky-400"></span>
            Use “Forçar conectar” para liberar bloqueios de estados pendentes
          </li>
        </ul>
        <div class="mt-5 rounded-xl border border-white/10 bg-white/5 p-4 text-sm space-y-1">
          <p class="text-xs uppercase tracking-widest text-white/60">Dica rápida</p>
          <p>Configure seus horários por dia para evitar conflitos: horários duplicados são consolidados automaticamente.</p>
        </div>
      </section>
