...
function isLikelyPhonePn(digits) {
...
function resolvePnFromContactStore(lidJid) {
    if (!lidJid || !sock || !sock.contacts || typeof sock.contacts !== "object") {
        return ""
    }
    const normalizedLid = String(lidJid).toLowerCase()
    const contact = Object.values(sock.contacts).find(entry => {
        const candidateLid = String(entry?.lid || "").toLowerCase()
        return candidateLid && candidateLid === normalizedLid
    })
    if (!contact) {
        return ""
    }
    const jid = String(contact.id || contact.jid || "").toLowerCase()
    if (jid.endsWith("@s.whatsapp.net")) {
        return normalizePhoneDigits(extractPhoneFromJid(jid))
    }
    return normalizePhoneDigits(contact.phone || contact.phoneNumber || "")
}

function buildIdentityFromMessage(msg) {
    const remoteJid = String(msg?.key?.remoteJid || "").toLowerCase()
    const participantJid = String(msg?.key?.participant || msg?.participant || "").toLowerCase()
    const contextInfo = msg?.message?.extendedTextMessage?.contextInfo || {}

    const lidCandidates = [
        remoteJid,
        participantJid,
        String(msg?.key?.remoteJidAlt || "").toLowerCase(),
        String(msg?.key?.participantAlt || "").toLowerCase(),
        String(msg?.lid || "").toLowerCase()
    ...
    if (remoteJid.endsWith("@s.whatsapp.net")) {
        rawPnCandidates.push(extractPhoneFromJid(remoteJid))
    }
    if (participantJid.endsWith("@s.whatsapp.net")) {
        rawPnCandidates.push(extractPhoneFromJid(participantJid))
    }

    const normalizedPn = rawPnCandidates
        .map(candidate => normalizePhoneDigits(candidate))
        .find(candidate => isLikelyPhonePn(candidate)) || ""

    const lid = lidCandidates[0] || null
    const pnFromStoreRaw = lid ? resolvePnFromContactStore(lid) : ""
    const pnFromStore = isLikelyPhonePn(pnFromStoreRaw) ? pnFromStoreRaw : ""
    const resolvedPn = normalizedPn || pnFromStore || null
    const canonicalRemoteJid = resolvedPn ? `${resolvedPn}@s.whatsapp.net` : (remoteJid || lid || null)
    const remoteJidAlt = lid && canonicalRemoteJid !== lid ? lid : null
...
async function persistMessageIdentity(msg) {
    if (!db) {
        return buildIdentityFromMessage(msg)
    }
    const identity = buildIdentityFromMessage(msg)
    if (!identity.remoteJid || !isIndividualJid(identity.remoteJid)) {
        return identity
    }

    try {
        if (identity.lid && identity.pn && typeof db.saveLIDPNMapping === "function") {
            await db.saveLIDPNMapping(INSTANCE_ID, identity.lid, identity.pn)
        }
        if (typeof db.updateContactIdentity === "function" && (identity.lid || identity.pn)) {
            await db.updateContactIdentity(INSTANCE_ID, identity.remoteJid, identity.lid, identity.pn)
            if (identity.canonicalRemoteJid && identity.canonicalRemoteJid !== identity.remoteJid) {
                await db.updateContactIdentity(INSTANCE_ID, identity.canonicalRemoteJid, identity.lid, identity.pn)
            }
        }
    } catch (err) {
        log("Erro ao persistir identidade do contato:", err.message)
    }
...
async function handleContactUpsert(contact) {
    ...
    const lid = String(contact.lid || "").toLowerCase().endsWith("@lid") ? String(contact.lid).toLowerCase() : null
    const pn = remoteJid.endsWith("@s.whatsapp.net") ? normalizePhoneDigits(extractPhoneFromJid(remoteJid)) : null
    ...
    if (db && (lid || pn)) {
        try {
            if (lid && pn && typeof db.saveLIDPNMapping === "function") {
                await db.saveLIDPNMapping(INSTANCE_ID, lid, pn)
            }
            if (typeof db.updateContactIdentity === "function") {
                await db.updateContactIdentity(INSTANCE_ID, remoteJid, lid, pn)
                if (lid) {
                    await db.updateContactIdentity(INSTANCE_ID, lid, lid, pn)
                }
            }
        } catch (err) {
            log("Erro ao salvar identidade de contacts.upsert:", err.message)
        }
    }

    await updateContactMetadata(remoteJid, { contactName, statusName, profilePicture })
    if (lid) {
        await updateContactMetadata(lid, { contactName, statusName, profilePicture })
    }
}
...
async function handleContactFromMessage(msg) {
    ...
    if (!isIndividualJid(remoteJid)) return
    const identity = await persistMessageIdentity(msg)
    const pushName = normalizeMetaField(msg.pushName)
    if (!pushName) return
    await updateContactMetadata(remoteJid, { contactName: pushName })
    if (identity?.canonicalRemoteJid && identity.canonicalRemoteJid !== remoteJid) {
        await updateContactMetadata(identity.canonicalRemoteJid, { contactName: pushName })
    }
}

async function syncContactStoreIdentities() {
    if (!db || !sock || !sock.contacts || typeof sock.contacts !== "object") {
        return
    }
    ...
    const tasks = contacts.map(async contact => {
        const remoteJid = String(contact?.id || contact?.jid || "").toLowerCase()
        const lid = String(contact?.lid || "").toLowerCase()
        ...
        const pn = normalizePhoneDigits(extractPhoneFromJid(remoteJid))
        if (!pn) {
            return
        }
        try {
            if (typeof db.saveLIDPNMapping === "function") {
                await db.saveLIDPNMapping(INSTANCE_ID, lid, pn)
            }
            if (typeof db.updateContactIdentity === "function") {
                await db.updateContactIdentity(INSTANCE_ID, remoteJid, lid, pn)
            ...
            log("Erro ao sincronizar identidade do contato:", err.message)
        }
    })
...
async function dispatchAIResponse(remoteJid, messageBody, providedConfig = null, options = { allowBoomerang: true }) {
    ...
    if (!outputText) {
        ...
        if (commands?.length && db) {
            const summaryText = notesText || "Funções executadas sem texto visível para o contato."
            try {
                await db.saveMessage(INSTANCE_ID, remoteJid, "assistant", summaryText, "outbound", metadata)
                log("flow.persist", {
                    remoteJid,
                    role: "assistant",
                    direction: "outbound",
                ...
                log("Error saving command-only assistant message:", err.message)
            }
        }
        return aiResponse
    }
    ...
    for (const [index, action] of segmentActions.entries()) {
        const segmentMetadata = metadata
...
async function processMessageWithAI(msg) {
        ...
        try {
            const identity = await persistMessageIdentity(msg)
            const storageRemoteJid = identity?.canonicalRemoteJid || remoteJid
            const storageOptions = {
                remoteJidAlt: identity?.remoteJidAlt || null,
                senderPn: identity?.senderPn || null
            }
            const persistUserInbound = async (content, metadata = null) => {
                if (!db) return
                await db.saveMessage(INSTANCE_ID, storageRemoteJid, "user", content, "inbound", metadata, storageOptions)
            }
            const persistAssistantOutbound = async (content, metadata = null) => {
                if (!db) return
                await db.saveMessage(INSTANCE_ID, storageRemoteJid, "assistant", content, "outbound", metadata, storageOptions)
            }
            ...
            if (db && typeof db.getTimeSinceLastInboundMessage === "function") {
                try {
                    lastInboundAt = await db.getTimeSinceLastInboundMessage(INSTANCE_ID, storageRemoteJid)
                } catch (err) {
            ...
            if (mediaPayload && mediaPayload.downloadType === "audio" && transcriptionConfig.enabled) {
                ...
                if (db) {
                    try {
                        await persistUserInbound(inboundText)
                    } catch (err) {
                ...
                try {
                    ...
                    if (sock) {
                        ...
                        if (db) {
                            try {
                                await persistAssistantOutbound(fallbackText)
                            } catch (saveError) {
                                log("Erro salvando mensagem de erro no banco:", saveError.message)
                            }
                        }
                ...
                if (db) {
                    try {
                        await persistAssistantOutbound(outgoingText)
                    } catch (err) {
            ...
            if (contactPayload) {
                ...
                if (db) {
                    try {
                        await persistUserInbound(promptText)
                        log("flow.persist", {
                            remoteJid: storageRemoteJid,
                            role: "user",
                            direction: "inbound",
            ...
            if (db) {
                try {
                    await persistUserInbound(messageBody, inboundMetadata)
                    log("flow.persist", {
                        remoteJid: storageRemoteJid,
                        role: "user",
                        direction: "inbound",
    ...
    try {
        ...
        sock.ev.on("connection.update", update => {
            ...
            if (connection === "open") {
                ...
                void syncContactStoreIdentities()
            }
...
app.post("/send-message", async (req, res) => {
    console.log("[DEBUG] /send-message hit. sock:", sock, "whatsappConnected:", whatsappConnected);
    if (!sock || !whatsappConnected) {
